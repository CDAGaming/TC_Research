...
public class MeshLoader
{
    ...
    private void loadMaterialLibrary(final ResourceLocation locOfParent, final String path) throws IOException {
        ...
        final ResourceLocation loc = new ResourceLocation(locOfParent.func_110624_b(), prefix + path);
        this.currentMatLib.loadFromStream(loc);
    }
    
    public MeshModel loadFromResource(final ResourceLocation loc) throws IOException {
        final IResource res = Minecraft.func_71410_x().func_110442_L().func_110536_a(loc);
        final InputStreamReader lineStream = new InputStreamReader(res.func_110527_b(), Charsets.UTF_8);
