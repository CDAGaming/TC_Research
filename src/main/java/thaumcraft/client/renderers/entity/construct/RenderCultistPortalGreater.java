...
public class RenderCultistPortalGreater extends Render
{
    ...
    public RenderCultistPortalGreater(final RenderManager renderManager) {
        super(renderManager);
        this.field_76989_e = 0.1f;
        this.field_76987_f = 0.5f;
    }
    
    public void renderPortal(final EntityCultistPortalGreater portal, final double px, double py, final double pz, final float par8, final float f) {
        final long nt = System.nanoTime();
        final long time = nt / 50000000L;
        float scaley = 1.5f;
        int e = (int)Math.min(50.0f, portal.field_70173_aa + f);
        ...
        final float bob = MathHelper.func_76126_a(portal.field_70173_aa / (5.0f - 12.0f * m)) * m + m;
        final float bob2 = MathHelper.func_76126_a(portal.field_70173_aa / (6.0f - 15.0f * m)) * m + m;
        final float alpha = 1.0f - bob;
        scaley -= bob / 4.0f;
        scale -= bob2 / 3.0f;
        this.func_110776_a(RenderCultistPortalGreater.portaltex);
        GL11.glPushMatrix();
        GL11.glEnable(3042);
