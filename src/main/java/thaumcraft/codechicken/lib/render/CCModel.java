...
import thaumcraft.codechicken.lib.lighting.*;
...
public class CCModel implements CCRenderState.IVertexSource, Copyable<CCModel>
{
        ...
        while ((line = reader.readLine()) != null) {
            ...
            if (!line.startsWith("#")) {
                ...
                if (line.startsWith("v ")) {
                    ...
                    ((ITransformation<Vector3, Transformation>)coordSystem).apply(vert);
                    verts.add(vert);
                }
    ...
    public static Map<String, CCModel> parseObjModels(final ResourceLocation res, final Transformation coordSystem) {
        try {
            return parseObjModels(Minecraft.func_71410_x().func_110442_L().func_110536_a(res).func_110527_b(), 4, coordSystem);
        }
        catch (IOException e) {
    ...
    public CCModel sidedCopy(final int side1, final int side2, final Vector3 point) {
        ...
        return model.apply(new TransformationList(new Transformation[] { ((ITransformation<Vector, Transformation>)Rotation.sideRotations[side1]).inverse(), Rotation.sideRotations[side2] }).at(point));
    }
    ...
    public CCModel generateSidedPart(final int side1, final int side2, final Vector3 point, final int srcpos, final int destpos, final int length) {
        return this.apply(new TransformationList(new Transformation[] { ((ITransformation<Vector, Transformation>)Rotation.sideRotations[side1]).inverse(), Rotation.sideRotations[side2] }).at(point), srcpos, destpos, length);
    }
