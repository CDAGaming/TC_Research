...
public class TransformationList extends Transformation
{
    ...
    public void apply(final Vector3 vec) {
        if (this.mat != null) {
            ...
            for (int i = 0; i < this.transformations.size(); ++i) {
                ((ITransformation<Vector3, Transformation>)this.transformations.get(i)).apply(vec);
            }
        }
    }
    ...
    private void compact() {
        ...
        while (iterator.hasNext()) {
            ...
            if (prev != null) {
                final Transformation m = ((ITransformation<Vector, Transformation>)prev).merge(t);
                if (m == null) {
                    newList.add(prev);
                }
    ...
    public Transformation inverse() {
        ...
        for (int i = this.transformations.size() - 1; i >= 0; --i) {
            rev.with(((ITransformation<Vector, Transformation>)this.transformations.get(i)).inverse());
        }
        return rev;
    }
